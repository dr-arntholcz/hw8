...
app.get('/api/products', (req, res) => {
    fs.readFile('./server/db/listProducts.json', 'utf-8', (err, data) => {
        if (err) {
...
app.delete('/api/cart/:id', (req, res) => {
    fs.readFile('./server/db/userCart.json', 'utf-8', (err, data) => {
        if (err) {
            ...
            const cart = JSON.parse(data);
            // ищем товар по id
            const indexProduct = cart.contents.findIndex(el => el.id === +req.params.id);
            cart.contents.splice(indexProduct, 1); //(req.body)
            // пишем обратно
            fs.writeFile('./server/db/userCart.json', JSON.stringify(cart), (err) => {
                if (err) {
                    res.send('{"result": 0}');
